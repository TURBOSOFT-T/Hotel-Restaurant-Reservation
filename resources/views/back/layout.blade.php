...
<div class="wrapper">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          @foreach(config('menu') as $name => $elements)
              @if($elements['role'] === 'manager' || auth()->user()->isAdmin())
                      ...
                      <li class="nav-item has-treeview {{ menuOpen($elements['children']) }}">
                          ...
                          <ul class="nav nav-treeview">
                              @foreach($elements['children'] as $child)
                                  @if(($child['role'] === 'manager' || auth()->user()->isAdmin()) && $child['name'] !== 'fake')
                                      <x-back.menu-item
                                          :route="$child['route']"
                                          :sub=true>
                                          @lang($child['name'])
  ...
  <footer class="main-footer">
    <!-- Default to the left -->
    <strong>Copyright &copy; 2023 {{ config('app.name', 'Laravel') }}.</strong>
  </footer>
</div>
<!-- ./wrapper -->
